//
//  CarServiceCollectionView.swift
//  Cario
//
...
class CarServiceCollectionView: NSObject, UICollectionViewDelegate, UICollectionViewDataSource,UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        let numberOfItem = LoginService.instance.carioInit?.services[0].service.count ?? 0
        return numberOfItem + 1
    }
    
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: CAR_SERVICE_COLLECTION_VIEW_CELL, for: indexPath) as! CarServiceCollectionViewCell
        if let services = LoginService.instance.carioInit?.services[0].service {
            if indexPath.row <= services.count - 1 {
                let service = services[indexPath.row]
                cell.configureCell(name: service.name, detail: service.description, badgeNumber: String(service.provider.count))
